...
games_per_year = df['year_of_release'].value_counts().sort_index(ascending=False)
...
platform_year_sales = df_recent.groupby(['year_of_release', 'platform'])['total_sales'].sum().reset_index() 

#Filtramos las plataformas mas exitosas
top_platforms = df_recent.groupby('platform')['total_sales'].sum().sort_values(ascending=False).head(10).index #Agregamos .index al final ya que solo necesitamos los nombres de las plataformas.
...
sns.lineplot(data=platform_year_sales_top, x='year_of_release', y='total_sales', hue='platform')
...
platform_trends = recent_years.groupby(['year_of_release', 'platform'])['total_sales'].sum().reset_index()

#Plataformas que son frecuentes desde 2010
top_recent_platforms = (platform_trends.groupby('platform')['total_sales'].sum().sort_values(ascending=False).head(6).index)
platform_trends_top = platform_trends[platform_trends['platform'].isin(top_recent_platforms)]

#Obtenemos la grafica
plt.figure(figsize=(12, 6))
sns.lineplot(data=platform_trends_top, x='year_of_release', y='total_sales', hue='platform')
...
recent_platforms_data = recent_years[recent_years['platform'].isin(active_platforms)]
...
ps4 = df_recent[df_recent['platform'] == 'PS4'][['user_score', 'critic_score', 'total_sales']].dropna()
...
for platform in platforms_to_check:
    subset = df_recent[df_recent['platform'] == platform][['user_score', 'critic_score', 'total_sales']].dropna()
...
genre_median_sales = df_recent.groupby('genre')['total_sales'].median().sort_values(ascending=False)
...
plt.subplot(1, 3, 1)
sns.barplot(x=genre_counts.values, y=genre_counts.index, alpha=0.7,palette='viridis')
...
#Ventas totales por genero
plt.subplot(1, 3, 2)
sns.barplot(x=genre_sales.values, y=genre_sales.index, alpha=0.7,palette='plasma')
...
plt.subplot(1, 3, 3)
sns.barplot(x=genre_mean_sales.values, y=genre_mean_sales.index, alpha=0.7,palette='magma')
...
#Norteamérica
df_recent.groupby('platform')['na_sales'].sum().sort_values(ascending=False).head(10).plot(kind='bar', ax=axes[0], color='skyblue')
axes[0].set_title('Ventas por plataforma en Norteamérica')
axes[0].set_ylabel('Ventas en millones')
axes[0].set_xlabel('Plataforma')

#Europa
df_recent.groupby('platform')['eu_sales'].sum().sort_values(ascending=False).head(10).plot(kind='bar', ax=axes[1], color='lightgreen')
axes[1].set_title('Ventas por plataforma en Europa')
axes[1].set_xlabel('Plataforma')

#Japón
df_recent.groupby('platform')['jp_sales'].sum().sort_values(ascending=False).head(10).plot(kind='bar', ax=axes[2], color='salmon')
...
#NA
df_recent.groupby('genre')['na_sales'].sum().sort_values(ascending=False).head(10).plot(kind='bar', ax=axes[0], color='mediumslateblue')
axes[0].set_title('Ventas por género en NA')
axes[0].set_ylabel('Ventas en millones')
axes[0].set_xlabel('Género')

#EU
df_recent.groupby('genre')['eu_sales'].sum().sort_values(ascending=False).head(10).plot(kind='bar', ax=axes[1], color='mediumseagreen')
axes[1].set_title('Ventas por género en EU')
axes[1].set_xlabel('Género')

#JP
df_recent.groupby('genre')['jp_sales'].sum().sort_values(ascending=False).head(10).plot(kind='bar', ax=axes[2], color='coral')
...
esrb_sales = df_recent.pivot_table(index='rating',values=['na_sales', 'eu_sales', 'jp_sales'], aggfunc='mean')
esrb_sales = esrb_sales[['na_sales','eu_sales','jp_sales']] #Para que salga ordenado el grafico
...
else: print("No rechazamos la hipótesis nula. Las calificaciones promedio de los usuarios son similares.")
